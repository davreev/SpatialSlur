 ...
 * Notes
 * 
 * TODO update to handle multi-edges as invalid state (no degree 2 faces or holes)
 */
...
namespace SpatialSlur.Meshes.Impl
{
        ...
        where TF : HeMesh<TV, TE, TF>.Face
    {
        ...
        /// </summary>
        [Serializable]
        public new abstract class Vertex : HeStructure<TV, TE, TF>.Vertex
        {
        }
        ...
        /// </summary>
        [Serializable]
        public new abstract class Halfedge : HeStructure<TV, TE, TF>.Halfedge
        {
            /// <summary>
            /// 
            /// </summary>
        ...
        /// </summary>
        [Serializable]
        public new abstract class Face : HeStructure<TV, TE, TF>.Face
        {
        }
        ...
        internal static void CheckTopology(HeMesh<TV, TE, TF> mesh)
        {
        ...
        private List<(TV, TE)> _addFaceBuffer = new List<(TV, TE)>();
        ...
        */
        ...
        {
            // TODO simplify check?
            // only need to prevent the creation of multi-edges
        ...
        */
        ...
        private void CleanupDegree2Hole(TE hedge)
        {
            ...
            if (v0.First == he0)
                v0.First = he0.NextBoundaryAtStart ?? he3;

            if (v1.First == he2)
                v1.First = he2.NextBoundaryAtStart ?? he2;
        ...
        private void CleanupDegree1Hole(TE hedge)
        {
            ...
            if (v0.First == he0)
                v0.First = he0.NextBoundaryAtStart ?? he1.Next;
        ...
        /// Assumes the given elements are valid for the operation.
        /// </summary>
        private bool SpinEdgeImpl(TE hedge)
        {
            var he0 = hedge;
            var he1 = he0.Twin;
            ...
            // avoids creation of multi-edge
            if (he2.Start.IsConnectedTo(he3.Start))
                return false;
            ...
            if (v1.First == he1) v1.First = he0.Next;
            ...
            if (he2 == f0.First) f0.First = he0;
            if (he3 == f1.First) f1.First = he1;
            ...
            // update halfedge->halfedge refs
            he2.MakeConsecutive(he1.Next);
            he3.MakeConsecutive(he0.Next);

            he2.Previous.MakeConsecutive(he0);
            he3.Previous.MakeConsecutive(he1);

            he0.MakeConsecutive(he3);
        ...
        /// </summary>
        /// <param name="vertex"></param>
        private void RemoveVertexInteriorImpl(TV vertex)
        {
            ...
            foreach(var he1 in he0.CirculateStart)
            {
                var he2 = he1.Next;
                var he3 = he1.Twin.Previous;
        ...
        */
        ...
        public TF AddFace(int vi0, int vi1, int vi2)
        {
            ...
            return Validate(vi0) && Validate(vi1) && Validate(vi2) ?
                AddFace(_addFaceBuffer) : null;

            bool Validate(int vi)
            {
                var v = Vertices[vi];

                // no duplicate vertices within a single face
                if (v.Tag == currTag)
                    return false;
        ...
        public TF AddFace(int vi0, int vi1, int vi2, int vi3)
        {
            ...
            return Validate(vi0) && Validate(vi1) && Validate(vi2) && Validate(vi3) ?
                AddFace(_addFaceBuffer) : null;
        ...
        public TF AddFace(TV v0, TV v1, TV v2)
        {
            ...
            return Validate(v0) && Validate(v1) && Validate(v2) ?
                AddFace(_addFaceBuffer) : null;
        ...
        public TF AddFace(TV v0, TV v1, TV v2, TV v3)
        {
            ...
            return Validate(v0) && Validate(v1) && Validate(v2) && Validate(v3) ? 
                AddFace(_addFaceBuffer) : null;
        ...
        private TF AddFace(List<(TV Vertex, TE Halfedge)> buffer)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // vertex must be on boundary
                if (!v.IsBoundary) return null;

                // check for existing halfedge between consecutive vertices
                var he = v.FindHalfedgeTo(buffer[(i + 1) % n].Vertex);
                if (he == null) continue;

                // if found, halfedge mustn't already have face
                if (!he.IsHole) return null;
        ...
        /// </summary>
        /// <param name="buffer"></param>
        private TF AddFaceImpl(List<(TV Vertex, TE Halfedge)> buffer)
        {
            // impl ref
        ...
        public void TriangulateFaces(IFaceTriangulator triangulator)
        {
            var nf = Faces.Count;

            for (int i = 0; i < nf; i++)
                triangulator.Triangulate(this, Faces[i].First);
        }
        ...
        public void TriangulateFaces(IFaceTriangulator triangulator, Func<TF, TE> getStart)
        {
            var nf = Faces.Count;

            for (int i = 0; i < nf; i++)
                triangulator.Triangulate(this, getStart(Faces[i]));
        }
        ...
        public void QuadrangulateFaces(IFaceQuadrangulator quadrangulator)
        {
            var nf = Faces.Count;

            for (int i = 0; i < nf; i++)
                quadrangulator.Quadrangulate(this, Faces[i].First);
        }
        ...
        /// <param name="quadrangulator"></param>
        /// <param name="getStart"></param>
        public void QuadrangulateFaces(IFaceQuadrangulator quadrangulator, Func<TF, TE> getStart)
        {
            var nf = Faces.Count;

            for (int i = 0; i < nf; i++)
                quadrangulator.Quadrangulate(this, getStart(Faces[i]));
        }

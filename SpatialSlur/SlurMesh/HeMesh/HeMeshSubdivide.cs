...
namespace SpatialSlur.SlurMesh
{
    ...
    public static class HeMeshSubdivide
    {
        ...
        /// <param name="mesh"></param>
        /// <param name="vertexPositions"></param>
        public static void QuadSplit(HeMesh mesh, List<Vec3d> vertexPositions)
        {
        ...
        /// <param name="mesh"></param>
        /// <param name="vertexPositions"></param>
        private static void QuadSplitGeom(HeMesh mesh, List<Vec3d> vertexPositions)
        {
            ...
            for (int i = 0; i < hedges.Count; i += 2)
            {
                ...
                if (he.IsUnused)
                    vertexPositions.Add(new Vec3d()); // add dummy vertex for unused elements
                else
                    vertexPositions.Add(he.Lerp(vertexPositions, 0.5));
            }
        }
        ...
        /// </summary>
        /// <param name="mesh"></param>
        private static void QuadSplitTopo(HeMesh mesh)
        {
            var hedges = mesh.Halfedges;
            ...
            for (int i = 0; i < nhe; i += 2)
            {
                ...
                if (he.IsUnused)
                    verts.Add(); // add placeholder
                else
                    hedges.SplitEdgeImpl(he); // split edge (adds a new vertex)
            }
            ...
            for (int i = 0; i < nf; i++)
            {
                ...
                {
                    var he2 = he1.Next;
                    var he3 = hedges.AddPair(he2.Start, fv);
 
                    HeMeshHalfedge.MakeConsecutive(he1, he3);
                    HeMeshHalfedge.MakeConsecutive(he3.Twin, he2);
                ...
                {
                    fv.First = he1.Next.Twin;
                    var he2 = he1.Previous;
  
                    do
                    {
                        var he3 = he1.Next;
                        var he4 = he2.Previous;
            ...
            for (int i = 0; i < nv; i++)
            {
                ...
                if (v.IsBoundary)
                {
                    ...
                    if (!he.IsAtDegree2)
                    {
                        ...
                        var p1 = vertexPositions[(he.Previous.Index >> 1) + nvf];
        ...
        private static HeMesh BevelEdgesTopo(HeMesh mesh, bool closeVertices, bool closeFaces)
        {
            ...
            var newMesh = new HeMesh(nhe << 2, nhe, nhe + nv + nf);
            var newVerts = newMesh.Vertices;
            var newFaces = newMesh.Faces;

            var fv = new List<HeMeshVertex>(4);
            fv.Fill();
        ...
        private static HeMesh FrameDualTopo(HeMesh mesh, bool closeVertices)
        {
            ...
            var newMesh = new HeMesh(nhe * 5, nhe, nhe + nf);
            var newVerts = newMesh.Vertices;
            var newFaces = newMesh.Faces;
        ...
        private static HeMesh WeaveTopo(HeMesh mesh)
        {
            ...
            var newMesh = new HeMesh(nhe << 3, nhe << 2, nhe << 2);
            var newVerts = newMesh.Vertices;
            var newFaces = newMesh.Faces;

...
namespace SpatialSlur.SlurMesh
{
    ...
    public static class HePhysics
    {
            ...
            Action<Tuple<int, int>> func = range =>
            {
                for (int i = range.Item1; i < range.Item2; i++)
                {
                    ...
                    if (v.IsBoundary)
                    {
                        ...
                        var vi1 = he.PrevInFace.Start.Index;
        ...
        /// <param name="boundaryType"></param>
        /// <param name="parallel"></param>
        public static void LaplacianSmooth(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> hedgeWeights, IList<Vec3d> forceSums, SmoothBoundaryType boundaryType = SmoothBoundaryType.Fixed, bool parallel = false)
        {
            _lapSmoothWeighted[(int)boundaryType](mesh, vertexPositions, strength, hedgeWeights, forceSums, parallel);
        }
        ...
        /// <param name="forceSums"></param>
        /// <param name="parallel"></param>
        private static void LaplacianSmoothFixed(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> hedgeWeights, IList<Vec3d> forceSums, bool parallel)
        {
            var verts = mesh.Vertices;
            ...
            Action<Tuple<int, int>> func = range =>
            {
                for (int i = range.Item1; i < range.Item2; i++)
                {
                    ...
                    foreach (var he in v.OutgoingHalfedges)
                        sum += he.GetDelta(vertexPositions) * hedgeWeights[he.Index];
        ...
        /// 
        /// </summary>
        private static void LaplacianSmoothCornerFixed(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> hedgeWeights, IList<Vec3d> forceSums, bool parallel)
        {
            var verts = mesh.Vertices;
        ...
        /// <param name="forceSums"></param>
        /// <param name="parallel"></param>
        private static void LaplacianSmoothFree(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> hedgeWeights, IList<Vec3d> forceSums, bool parallel)
        {
            var verts = mesh.Vertices;
        ...
        public static void CirclePack(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IList<Vec3d> forceSums)
        {
            ...
            {
                var he = hedges[i];
                if (he.IsRemoved || he.IsBoundary) continue; // skip unused, boundary, or edges

                // collect relevant edges
                var he0 = he.NextInFace;
                var he1 = he.PrevInFace;
                var he2 = he.Twin.NextInFace;
                var he3 = he.Twin.PrevInFace;
        ...
        public static void CirclePack(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IReadOnlyList<double> vertexRadii, IList<Vec3d> forceSums)
        {
            ...
            {
                var he = edges[i];
                if (he.IsRemoved) continue; // skip unused edges
        ...
        public static void EqualizeFaceEdgeLengths(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IList<Vec3d> forceSums)
        {
            ...
            {
                if (f.IsRemoved) continue; // skip unused faces
        ...
        public static void PlanarizeFaces(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IList<Vec3d> forceSums)
        {
            foreach (var f in mesh.Faces)
            {
                ...
                var vi0 = he1.Start.Index; he1 = he1.NextInFace;
                var vi1 = he1.Start.Index; he1 = he1.NextInFace;
                var vi2 = he1.Start.Index; he1 = he1.NextInFace;
        ...
        public static void PlanarizeQuads(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IList<Vec3d> forceSums)
        {
            foreach (var f in mesh.Faces)
            {
                ...
                var vi1 = he1.Start.Index; he1 = he1.NextInFace;
                var vi2 = he1.Start.Index; he1 = he1.NextInFace;
                if (he1.NextInFace != he0) continue; // skip non quads
        ...
        /// <param name="strength"></param>
        /// <param name="edgeLengths"></param>
        /// <param name="hedgeAngles"></param>
        /// <param name="forceSums"></param>
        public static void Circularize(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IReadOnlyList<double> hedgeAngles, IList<Vec3d> forceSums)
        {
            foreach (var f in mesh.Faces)
            {
                if (f.IsRemoved) continue; // skip unused faces
                ...
                var he3 = he2.NextInFace;
                if (he3.NextInFace != he0) continue; // skip non quads

                for (int i = 0; i < 2; i++)
                {
                    // calculate angle error
                    double err = (Math.PI - hedgeAngles[he1.Index] - hedgeAngles[he3.Index]) * 0.25; // angle error in range [-PI/4, PI/4]
        ...
        /// <param name="hedgeAngles"></param>
        /// <param name="forceSums"></param>
        public static void Conicalize(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IReadOnlyList<double> hedgeAngles, IList<Vec3d> forceSums)
        {
            var verts = mesh.Vertices;
            ...
            {
                var v = verts[i];
                if (v.IsRemoved || v.IsBoundary) continue; // skip unused or boundary vertices

                // conical rule - 2 pairs of opposite angles must have an equal sum
                // http://www.geometrie.tugraz.at/wallner/quad06.pdf
                // http://vecg.cs.ucl.ac.uk/Projects/SmartGeometry/constrained_mesh/paper_docs/constrainedMesh_sigA_11.pdf
                var he0 = v.FirstOut;
                var he1 = he0.Twin.NextInFace;
                var he2 = he1.Twin.NextInFace;
                var he3 = he2.Twin.NextInFace;
                if (he3.Twin.NextInFace != he0) continue; // skip non degree 4 verts

                // get angle error
                double err = ((hedgeAngles[he0.Index] + hedgeAngles[he2.Index]) - (hedgeAngles[he1.Index] + hedgeAngles[he3.Index])) * 0.25; // angle error in range [-PI/4, PI/4]
                double tan = Math.Tan(err) * strength;
        ...
        /// <param name="hedgeAngles"></param>
        /// <param name="forceSums"></param>
        public static void Developablize(HeMesh mesh, IReadOnlyList<Vec3d> vertexPositions, double strength, IReadOnlyList<double> edgeLengths, IReadOnlyList<double> hedgeAngles, IList<Vec3d> forceSums)
        {
            foreach (var v in mesh.Vertices)
            {
                if (v.IsRemoved || v.IsBoundary) continue; // skip unused or boudnary vertices

                // calculate angle error
                double sum = 0.0;
                foreach (var he in v.OutgoingHalfedges) 
                    sum += Math.PI - hedgeAngles[he.Index];
